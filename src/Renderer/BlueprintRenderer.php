        ...
        try {
            ...
            if (!isset($GLOBALS['AVISOTA_MAILCHIMP_TEMPLATE'][$templateGroup][$templateName])) {
                throw new \RuntimeException(
                    'Mailchimp blueprint ' . $templateGroup . '/' . $templateName . ' was not found!'
                );
            }
            ...
            foreach ($cells as $cellName => $cellConfig) {
                ...
                // remove empty nodes
                if (is_array($cellContents[$cellName])
                    ...
                    if (isset($cellConfig['ifEmptyRemove'])) {
                        ...
                        if (isset($cellConfig['xpath'])) {
                            ...
                            throw new \RuntimeException(
                                sprintf(
                                    'The cell "%s" does not have a valid selector',
                                    $cellName
                                )
                            );
                        }
                    ...
                    if (!$nodes->length) {
                        throw new \RuntimeException(
                            'Node "' . $expression . '" not found in ' . $blueprint['template']
                        );
                    }
                    ...
                    if (!empty($cellContents[$cellName])) {
                        ...
                        $targetNodes = $xpath->query(
                            str_replace('mc:', 'mc__', $expression),
